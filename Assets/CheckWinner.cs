...
public class CheckWinner : MonoBehaviour
{
        ...
        private void LateUpdate()
    {
        if(target != null && isWinner)
            {
                Vector3  desiredPoition = new Vector3(target.position.x, target.position.y, target.position.z + 2.2f);

                Vector3 smoothedPosition = Vector3.Lerp(winnerCamera.transform.position, desiredPoition
    ...
    {
        if (other.CompareTag("Player") && plaecontroller.instance.groundedPlayer)
        {
            isWinner = true;
        }
    }
